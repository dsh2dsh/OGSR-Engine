...
#pragma warning(disable : 4913) // warning C4913: пользовательский двоичный оператор "," существует, но ни одной перегрузке не удалось преобразовать все операнды, по умолчанию
                                // использован встроенный двоичный оператор "," (компилируется исходный файл ui\UIMainIngameWnd.cpp)
    for (int i = 0; I != E; ++I, ++i)
    {
#pragma warning(pop)
        stream.open_chunk(i);
