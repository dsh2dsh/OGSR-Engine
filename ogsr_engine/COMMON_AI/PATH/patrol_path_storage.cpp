    ...
    if ( on_level || ( ai().game_graph().valid_vertex_id( pp.m_game_vertex_id ) && ai().game_graph().vertex( pp.m_game_vertex_id )->level_id() == ai().level_graph().level_id() ) ) {
      ...
      if ( strict_pos_inside && !ai().level_graph().inside( pp.m_level_vertex_id, pp.m_position ) ) {
        ...
        if ( ai().level_graph().valid_vertex_id( new_vertex_id ) ) {
          ...
          Msg( "* [%s]: path[%s] pp[%s] fix level_vertex_id[%u] -> %u by position", __FUNCTION__, patrol_name.c_str(), pp.m_name.c_str(), prev_vertex_id, pp.m_level_vertex_id );
        }
        else {
          Fvector pos = ai().level_graph().vertex_position( pp.m_level_vertex_id );
          Msg( "* [%s]: path[%s] pp[%s] fix position to be inside level_vertex_id[%u]", __FUNCTION__, patrol_name.c_str(), pp.m_name.c_str(), pp.m_level_vertex_id );
