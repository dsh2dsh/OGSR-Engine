...
void prefetch_module(const char* file_name) { ai().script_engine().process_file(file_name); }
...
{
    module(
        L)[def("log1", (void (*)(LPCSTR)) & Log), def("fail", (void (*)(LPCSTR)) & msg_and_fail),
           ...
           def("prefetch", &prefetch_module), def("editor", &editor), def("bit_and", &bit_and), def("bit_or", &bit_or), def("bit_xor", &bit_xor), def("bit_not", &bit_not),
           def("user_name", &user_name), def("time_global", &script_time_global),
           // функции из ogse.dll
           def("GetShift", &GetShift), def("GetLAlt", &GetLAlt), def("GetRAlt", &GetRAlt), def("GetAlt", &GetAlt), def("device", &get_device)];
}
