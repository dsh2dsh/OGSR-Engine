...
bool CUIInventoryWnd::OnItemDrop(CUICellItem* itm)
{
    ...
    // В пределах одного владельца перемещать можно только в том случае, если
    // это пояс. Т.е. между ячейками пояса. Иначе будет выглядеть
    // странно. Например, если мы сдвинем предмет в слоте, т.е. чуть-чуть его
    // протащим и бросим в этом же слоте, будет звук, как будто мы перенесли в
    // этот слот предмет.
    if (old_owner == new_owner && t_new != iwBelt)
        return false;
