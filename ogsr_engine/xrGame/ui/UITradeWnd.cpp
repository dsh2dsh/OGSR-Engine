...
CUITradeWnd::CUITradeWnd() : m_bDealControlsVisible(false), m_pTrade(NULL), m_pOthersTrade(NULL), bStarted(false)
{
    ...
    hide_untradeable_cond = READ_IF_EXISTS(pSettings, r_bool, "trade", "hide_untradeable_cond", hide_untradeable);
}
...
{
    if (CanMoveToOther((PIItem)m_pCurrentCellItem->m_pData, true) != eItemMoveOK)
        return false;
...
{
    if (CanMoveToOther((PIItem)m_pCurrentCellItem->m_pData, false) != eItemMoveOK)
        return false;
...
void CUITradeWnd::MoveItems(CUICellItem* itm)
{
    ...
    {
        if (CanMoveToOther((PIItem)itm->m_pData, true) != eItemMoveOK)
            return;
    ...
    {
        if (CanMoveToOther((PIItem)itm->m_pData, false) != eItemMoveOK)
            return;
