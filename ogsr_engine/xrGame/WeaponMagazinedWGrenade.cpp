...
void CWeaponMagazinedWGrenade::Load(LPCSTR section)
{
    ...
    animGetEx(mhud_idle_sprint_g,
              pSettings->line_exist(hud_sect.c_str(), "anim_idle_sprint_g")   ? "anim_idle_sprint_g" :
    ...
    animGetEx(mhud_idle_sprint_gl,
              pSettings->line_exist(hud_sect.c_str(), "anim_idle_sprint_gl")  ? "anim_idle_sprint_gl" :
                  pSettings->line_exist(hud_sect.c_str(), "anim_idle_sprint") ? "anim_idle_sprint" :
                                                                                "anim_idle_gl");
...
bool CWeaponMagazinedWGrenade::TryPlayAnimIdle(u8 state = eIdle)
{
    ...
    if (IsGrenadeLauncherAttached() && !IsZoomed())
    {
        switch (state)
        {
        case eSubstateIdleMoving: m_pHUD->animPlay(random_anim(m_bGrenadeMode ? mhud_idle_moving_g : mhud_idle_moving_gl), TRUE, NULL, GetState()); return true;
        case eSubstateIdleSprint: m_pHUD->animPlay(random_anim(m_bGrenadeMode ? mhud_idle_sprint_g : mhud_idle_sprint_gl), TRUE, NULL, GetState()); return true;
