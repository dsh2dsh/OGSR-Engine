...
void stalker_movement_manager_base::on_restrictions_change()
{
    ...
    if (!object().getDestroy() && m_current.desired_position() &&
        (!ai().level_graph().valid_vertex_position(*m_current.desired_position()) || !restrictions().accessible(*m_current.desired_position())))
        set_nearest_accessible_position();
}
