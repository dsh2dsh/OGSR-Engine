...
void CWeaponPistol::Load(LPCSTR section)
{
    ...
    animGetEx(mhud_pistol.mhud_idle_sprint_empty,
              pSettings->line_exist(hud_sect.c_str(), "anim_idle_sprint_empty") ? "anim_idle_sprint_empty" :
                  pSettings->line_exist(hud_sect.c_str(), "anim_idle_sprint")   ? "anim_idle_sprint" :
                                                                                  "anim_empty");
    animGetEx(mhud_pistol.mhud_idle_moving_empty,
              pSettings->line_exist(hud_sect.c_str(), "anim_idle_moving_empty") ? "anim_idle_moving_empty" :
    ...
    animGetEx(mhud_pistol_r.mhud_idle_sprint_empty,
              pSettings->line_exist(hud_sect.c_str(), "anim_idle_sprint_empty") ? "anim_idle_sprint_empty" :
                  pSettings->line_exist(hud_sect.c_str(), "anim_idle_sprint")   ? "anim_idle_sprint" :
                                                                                  "anim_empty");
    animGetEx(mhud_pistol_r.mhud_idle_moving_empty,
              pSettings->line_exist(hud_sect.c_str(), "anim_idle_moving_empty") ? "anim_idle_moving_empty" :
                  pSettings->line_exist(hud_sect.c_str(), "anim_idle_moving")   ? "anim_idle_moving" :
                                                                                  "anim_empty");
...
bool CWeaponPistol::TryPlayAnimIdle(u8 state = eIdle)
{
    ...
    if (m_opened)
    {
        if (!IsZoomed())
        {
            ...
            switch (state)
            {
            case eSubstateIdleMoving: m_pHUD->animPlay(random_anim(m.mhud_idle_moving_empty), TRUE, NULL, GetState()); return true;
            case eSubstateIdleSprint: m_pHUD->animPlay(random_anim(m.mhud_idle_sprint_empty), TRUE, NULL, GetState()); return true;
