        ...
        /*
                if ( objective.object_story_id.size() ) {
                    ...
                    if ( objective.object_id == u16(-1) )
                      Msg( "! [%s]: can't find object_id by object_story_id[%s] objective.description[%s] m_Title[%s]", __FUNCTION__, objective.object_story_id.c_str(),
           objective.description.c_str(), m_Title.c_str() );
                }
        */
...
SGameTaskObjective::SGameTaskObjective(CGameTask* parent, int _idx)
    : description(NULL), article_id(NULL), map_location(NULL), object_story_id(nullptr), object_id(u16(-1)), task_state(eTaskStateInProgress), def_location_enabled(true),
      parent(parent), idx(_idx)
{}
...
u16 SGameTaskObjective::get_object_id(bool strict)
{
...
#ifdef CRASH_ON_INVALID_STORY_ID
        ASSERT_FMT(id != u16(-1) || !strict, "[%s]: can't find object_id by object_story_id[%s] description[%s] m_Title[%s]", __FUNCTION__, object_story_id.c_str(),
                   description.c_str(), parent ? parent->m_Title.c_str() : "");
        return id;
#else
        if (id == u16(-1) && strict)
            Msg("! [%s]: can't find object_id by object_story_id[%s] description[%s] m_Title[%s]", __FUNCTION__, object_story_id.c_str(), description.c_str(),
                parent ? parent->m_Title.c_str() : "");
        else
            return id;
#endif
    }
...
CMapLocation* SGameTaskObjective::LinkedMapLocation(bool strict)
{
    if (map_location.size() == 0)
        return nullptr;
    u16 id = get_object_id(strict);
    if (!strict && id == u16(-1))
        return nullptr;
    return Level().MapManager().GetMapLocation(map_location, id);
}
