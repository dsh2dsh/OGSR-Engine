...
void CCar::SWheelDrive::Init()
{
    ...
    if (pos_fvd_debug)
    {
        ...
        {
        case jtWheel: Msg("[%s]: %s mXFORM.k.x[%f]", __FUNCTION__, pwheel->car->cName().c_str(), bone_map.find(pwheel->bone_id)->second.element->mXFORM.k.x); break;
        default: NODEFAULT;
        }
    }
}
...
void CCar::SWheelSteer::Init()
{
    ...
    if (pos_right_debug)
    {
        ...
        switch (bone_data.IK_data.type)
        {
        case jtWheel: Msg("[%s]: %s mXFORM.i.y[%f]", __FUNCTION__, pwheel->car->cName().c_str(), bone_map.find(pwheel->bone_id)->second.element->mXFORM.i.y); break;
        default: NODEFAULT;
        }
    }
