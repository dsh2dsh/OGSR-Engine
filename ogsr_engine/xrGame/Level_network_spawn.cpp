...
void CLevel::ProcessGameSpawns()
{
    ...
    if (trader)
    {
        for (auto& E : game_spawn_queue)
        {
            ...
            {
                // MsgIfDbg( "* [%s]: delayed spawn dwFrame[%u] trader[%d] ID[%d] ID_Parent[%d] name_replace[%s]", __FUNCTION__, Device.dwFrame, trader->ID, E->ID, E->ID_Parent,
                // E->name_replace() );
                g_sv_Spawn(E);
            }
        }
    ...
    game_spawn_queue.erase(std::remove_if(game_spawn_queue.begin(), game_spawn_queue.end(),
                                          [&](auto& E) {
                                              if (type == GE_DESTROY)
                                              {
                                                  ...
                                                  {
                                                      // MsgIfDbg( "* [CLevel::ProcessGameSpawnsDestroy]: delayed spawn GE_DESTROY dest[%d] ID[%d] ID_Parent[%d] name_replace[%s]",
                                              ...
                                              { // type == GE_DESTROY_REJECT
                                                  // MsgIfDbg( "* [CLevel::ProcessGameSpawnsDestroy]: delayed spawn GE_DESTROY_REJECT dest[%d] ID[%d] ID_Parent[%d]
                                                  // name_replace[%s]", dest, E->ID, E->ID_Parent, E->name_replace() );
                                                  F_entity_Destroy(E);
                                                  return true;
                                              }
                                              return false;
