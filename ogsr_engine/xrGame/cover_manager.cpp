...
void CCoverManager::compute_static_cover()
{
    ...
    for (u32 i = 0; i < n; ++i)
        if (ai().level_graph().vertex(i)->cover(0) +
            ai().level_graph().vertex(i)->cover(1) +
            ai().level_graph().vertex(i)->cover(2) +
            ai().level_graph().vertex(i)->cover(3))
...
#ifdef DEBUG
    MsgIfDbg("[%s] compute time (%zi): [%.2f s.]", __FUNCTION__, n,
