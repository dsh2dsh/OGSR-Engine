...
imotion_position::imotion_position()
    : interactive_motion(), time_to_end(0.f), saved_visual_callback(nullptr), blend(nullptr), shell_motion_has_history(false){
...
void imotion_position::state_start()
{
    ...
    VERIFY_FORMAT(B.stop_at_end, "can not use cyclic anim in death animth motion: %s", KA->LL_MotionDefName_dbg(B.motionID).first);
