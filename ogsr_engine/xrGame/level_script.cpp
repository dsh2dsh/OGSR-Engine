...
void g_set_detector_params(int _one, int _two) { shader_exports.set_detector_params(Ivector2().set(_one, _two)); }
...
CPatrolPath* get_patrol_path(LPCSTR patrol_path) { return ai().patrol_paths_raw().get_path(shared_str(patrol_path)); }
...
bool is_actor_indoor() { return Level().IsActorIndoor(); }
...
void CLevel::script_register(lua_State* L)
{
    module(L)[class_<CEnvDescriptor>("CEnvDescriptor")
        ...
        module(L, "level")[
            ...
            def("get_time_days", get_time_days), def("get_time_hours", get_time_hours), def("get_time_minutes", get_time_minutes), def("get_time_seconds", get_time_seconds),
            ...
            def("iterate_vertices_border", &iterate_vertices_border), def("get_character_community_team", &get_character_community_team), def("is_actor_indoor", &is_actor_indoor),
            def("get_effector_bobbing", &get_effector_bobbing), def("get_patrol_path", &get_patrol_path), def("map_iterate_user_spots_text", &map_iterate_user_spots_text),
            def("pre_init_inventory", &pre_init_inventory), def("is_ray_intersect_sphere", &is_ray_intersect_sphere),
    ...
    module(L)[def("set_artefact_slot", &g_set_artefact_position), def("set_anomaly_slot", &g_set_anomaly_position), def("set_detector_mode", &g_set_detector_params),
              def("update_inventory_window", &update_inventory_window), def("update_inventory_weight", &update_inventory_weight),
