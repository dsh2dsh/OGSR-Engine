                                                ...
                                                const luabind::functor<bool>& callback)
{
    ...
    ASSERT_FMT(stalker, "[%s]: %s not a CAI_Stalker", __FUNCTION__, object().cName().c_str());
    std::vector<const CCoverPoint*> covers;
    stalker->m_ce_far->setup(enemy_position, min_distance, max_distance, deviation, [&](auto point) -> bool {
        covers.push_back(point);
        return true;
    });
                                                  ...
                                                  const luabind::functor<bool>& callback)
{
    ...
    ASSERT_FMT(stalker, "[%s]: %s not a CAI_Stalker", __FUNCTION__, object().cName().c_str());
    std::vector<const CCoverPoint*> covers;
    stalker->m_ce_close->setup(enemy_position, min_distance, max_distance, deviation, [&](auto point) -> bool {
        covers.push_back(point);
        return true;
    });
