...
void CMemoryManager::update(const std::deque<T>& objects, bool add_enemies)
{
    ...
    std::deque<T>::const_iterator I = objects.begin();
    std::deque<T>::const_iterator E = objects.end();
    for (; I != E; ++I)
    {
        if (!(*I).m_enabled)
            continue;
...
void CMemoryManager::remove_links(CObject* object) { remove_object(object, true); }
