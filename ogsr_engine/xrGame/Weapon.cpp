...
CWeapon::~CWeapon() { xr_delete(m_UIScope); }
...
{
    string256 full_name;
    if (Core.Features.test(xrCore::Feature::ogse_wpn_zoom_system) && is_second_zoom_offset_enabled //Если включен режим второго прицеливания
...
void CWeapon::UpdateHUDAddonsVisibility(bool is_activating)
{
    if (H_Parent() != Level().CurrentEntity()) // actor only
        return;
    if (m_pHUD->IsHidden() && !is_activating)
        return;
...
bool CWeapon::IsPartlyReloading() { return (m_set_next_ammoType_on_reload == u32(-1) && GetAmmoElapsed() > 0 && !IsMisfire()); }

bool CWeapon::use_crosshair() const { return psActorFlags.test(AF_CROSSHAIR_DBG) || !psHUD_Flags.test(HUD_CROSSHAIR_ZOOM); }

void CWeapon::ReloadNextAmmo(u32 next_ammoType)
{
    ASSERT_FMT(next_ammoType < m_ammoTypes.size(),
               "[%s]: wrong next_ammoType[%u] >= m_ammoTypes[%u]", __FUNCTION__,
               next_ammoType, m_ammoTypes.size());
    ASSERT_FMT(unlimited_ammo() ||
                   m_pCurrentInventory->GetAmmo(
                       m_ammoTypes[next_ammoType].c_str(), ParentIsActor()),
               "[%s]: ammo of next_ammoType[%u] not found", __FUNCTION__,
               next_ammoType);
