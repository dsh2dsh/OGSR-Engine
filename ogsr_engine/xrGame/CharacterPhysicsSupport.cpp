...
CCharacterPhysicsSupport::CCharacterPhysicsSupport(EType atype, CEntityAlive* aentity)
    : m_pPhysicsShell(aentity->PPhysicsShell()), m_EntityAlife(*aentity), mXFORM(aentity->XFORM()), m_ph_sound_player(aentity), m_interactive_motion(nullptr),
      ...
      m_BonceDamageFactor(1.f), m_collision_hit_callback(nullptr), m_interactive_animation(NULL), m_physics_shell_animated(NULL), m_physics_shell_animated_time_destroy(u32(-1)),
      m_weapon_attach_bone(0), m_active_item_obj(0), m_hit_valide_time(u32(-1)), m_collision_activating_delay(nullptr)
{
    m_flags.assign(0);
    ...
    case etStalker:
        ...
        break;
    case etBitting: m_PhysicMovementControl->AllocateCharacterObject(CPHMovementControl::ai); m_PhysicMovementControl->SetRestrictionType(CPHCharacter::rtMonsterMedium);
    }
};
...
void CCharacterPhysicsSupport::in_shedule_Update(u32 DT)
{
    if (m_collision_activating_delay)
        Device.add_to_seq_parallel(fastdelegate::MakeDelegate(this, &CCharacterPhysicsSupport::UpdateCollisionActivatingDellay));
...
void CCharacterPhysicsSupport::CreateShell(CObject* who, Fvector& dp, Fvector& velocity)
{
    ...
    if (K->LL_BoneCount() > 0)
        for (int I = K->LL_BoneCount() - 1; I >= 0; --I) //-V621
