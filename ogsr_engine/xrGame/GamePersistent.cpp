        ...
        if (env_amb)
        {
            ...
            for (u32 idx = 0; I != E; ++I, ++idx)
            {
                ...
                else if (Device.dwTimeGlobal > ambient_sound_next_time[idx])
                {
                    ...
                    ambient_sound_next_time[idx] = Device.dwTimeGlobal +
                        _length_ms + ch.get_rnd_sound_time();
            ...
            /*
                        if (Device.dwTimeGlobal > ambient_sound_next_time)
                        {
                            ...
                            ambient_sound_next_time	= Device.dwTimeGlobal +
...
void CGamePersistent::WeathersUpdate()
{
    if (g_pGameLevel && !g_dedicated_server)
    {
        ...
        if (env_amb)
        {
            ...
            {
                ref_sound* snd = env_amb->get_rnd_sound();
                ambient_sound_next_time =
                    Device.dwTimeGlobal + env_amb->get_rnd_sound_time();
                if (snd)
                {
                    Fvector pos;
                    float angle = ::Random.randF(PI_MUL_2);
                    ...
                    pos.z = _sin(angle);
                    pos.normalize()
                        .mul(env_amb->get_rnd_sound_dist())
                        .add(Device.vCameraPosition);
                    pos.y += 10.f;
                    snd->play_at_pos(0, pos);
                }
