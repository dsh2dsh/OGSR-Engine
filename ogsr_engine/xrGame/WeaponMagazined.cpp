...
bool CWeaponMagazined::TryPlayAnimIdle(u8 state = eIdle)
{
    ...
    if (!IsZoomed())
    {
        switch (state)
        {
        case eSubstateIdleMoving: m_pHUD->animPlay(random_anim(mhud.mhud_idle_moving), TRUE, NULL, GetState()); return true;
        case eSubstateIdleSprint: m_pHUD->animPlay(random_anim(mhud.mhud_idle_sprint), TRUE, NULL, GetState()); return true;
