...
MotionID CStalkerAnimationManager::missile_animation(u32 slot, const EBodyState& body_state)
{
    ...
    {
    case MS_SHOWING: return (torso().select(animation[0].A));
    case MS_HIDING: return (torso().select(animation[3].A));
    case MS_THREATEN: return (animation[1].A[0]);
    case MS_READY: return (animation[1].A[1]);
    case MS_THROW: return (animation[1].A[2]);
    case MS_END: return (animation[1].A[2]);
    case MS_PLAYING: return (animation[1].A[2]);
    case MS_IDLE:
    case MS_HIDDEN:
    case MS_EMPTY:
