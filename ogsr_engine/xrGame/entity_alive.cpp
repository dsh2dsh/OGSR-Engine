...
void CEntityAlive::Hit(SHit* pHDS)
{
    ...
    if (HDS.hit_type == ALife::eHitTypeFireWound || HDS.hit_type == ALife::eHitTypeWound)
    {
        ...
        if (!use_simplified_visual() &&
            (smart_cast<CActor*>(HDS.who) || (Core.Features.test(xrCore::Feature::npc_simplified_shooting) && Device.vCameraPosition.distance_to(Position()) < g_shotmarks_dist)))
    ...
    case SBoneShape::stCylinder: {
        ...
        else
        {
            ...
            height = shape.cylinder.m_height / 2.f *
                ((normalized_value < shape.cylinder.m_radius / 2.f) ? -1.f :
                                                                      1.f);
            radius = height > 0.f ?
