...
CWeaponKnife::~CWeaponKnife() { HUD_SOUND::DestroySound(m_sndShot); }
...
void CWeaponKnife::StartIdleAnim() { m_pHUD->animDisplay(mhud_idle[Random.randI(mhud_idle.size())], TRUE); }
...
void CWeaponKnife::PlayAnimIdle(u8 state)
{
    ...
    switch (state)
    {
    case eSubstateIdleMoving: m_pHUD->animPlay(random_anim(mhud_idle_moving), TRUE, this, GetState()); break;
    case eSubstateIdleSprint: m_pHUD->animPlay(random_anim(mhud_idle_sprint), TRUE, this, GetState()); break;
    default: m_pHUD->animPlay(random_anim(mhud_idle), TRUE, this, GetState());
    }
}
