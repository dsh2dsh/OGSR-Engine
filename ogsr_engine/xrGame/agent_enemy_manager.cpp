...
CAgentEnemyManager::~CAgentEnemyManager() { cleanup_wounded_processors(); }
...
void CAgentEnemyManager::fill_enemies()
{
...

    {
        for (int i = 0, n = (int)m_wounded.size(); i < n; ++i)
        {
            ...
            m_wounded_processors.erase(std::remove_if(m_wounded_processors.begin(), m_wounded_processors.end(), find_wounded_predicate(enemy)), m_wounded_processors.end());
        }
    }
...
{
    std::deque<T>::iterator I = std::find(objects.begin(), objects.end(), enemy.m_object->ID());
    if (I != objects.end())
    {
    ...
    while (population(assigned) < combat_member_count)
    {
        ...
        else
        {
            ...
            if (stalker)
            {
                ...
                if (best_distance_sqr < dist || !stalker->g_Alive() || stalker->wounded())
                {
                    ASSERT_FMT(stalker->get_agent_manager(), "[%s]: %s has no agent_manager()", __FUNCTION__, stalker->cName().c_str());
...
void CAgentEnemyManager::remove_links(CObject* object)
{
    ...
    m_wounded_processors.erase(std::remove_if(m_wounded_processors.begin(), m_wounded_processors.end(), wounded_predicate(object)), m_wounded_processors.end());
}
...
{
    m_wounded.erase(std::remove_if(m_wounded.begin(), m_wounded.end(), find_wounded_predicate(object)), m_wounded.end());
    m_wounded_processors.erase(std::remove_if(m_wounded_processors.begin(), m_wounded_processors.end(), find_wounded_predicate(object)), m_wounded_processors.end());
}

void CAgentEnemyManager::remove_wounded_processor(ALife::_OBJECT_ID wounded_processor_id)
{
    m_wounded.erase(std::remove_if(m_wounded.begin(), m_wounded.end(), [&](const auto& it) -> bool { return it.second.first == wounded_processor_id; }), m_wounded.end());
    m_wounded_processors.erase(
        std::remove_if(m_wounded_processors.begin(), m_wounded_processors.end(), [&](const auto& it) -> bool { return it.second.first == wounded_processor_id; }),
        m_wounded_processors.end());
}

void CAgentEnemyManager::cleanup_wounded_processors()
{
    for (const auto& it : m_wounded)
        m_wounded_processors.erase(std::remove_if(m_wounded_processors.begin(), m_wounded_processors.end(), find_wounded_predicate(it.first)), m_wounded_processors.end());
}
