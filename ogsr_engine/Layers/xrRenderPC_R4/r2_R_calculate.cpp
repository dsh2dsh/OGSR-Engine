...
// https://github.com/OpenXRay/xray-16/commit/7b52e5cfc1004e8ff4e36ec8812431e4d4e04af5
...
void CRender::Calculate()
{
    ...
    if (!vLastCameraPos.similar(Device.vCameraPosition, DELTA_CAMERA_POS))
    {
        ...
        if (rmPortals)
        {
            ...
            Sectors_xrc.box_query(rmPortals, Device.vCameraPosition,
                                  box_radius);
            for (int K = 0; K < Sectors_xrc.r_count(); K++)
            {
                CPortal* pPortal = (CPortal*)
                    Portals[rmPortals->get_tris()[Sectors_xrc.r_begin()[K].id]
                                .dummy];
