...
void CRenderTarget::PhaseRainDrops()
{
    ...
    if (fis_zero(rain_drops_factor))
        ...
        // Msg("##[%s] rain_drops_factor: [%f], rain_density: [%f]",
        // __FUNCTION__, rain_drops_factor,
        // g_pGamePersistent->pEnvironment->CurrentEnv->rain_density);
...
#endif
    Fvector4 params{rain_drops_factor, ps_r2_rain_drops_intensity,
                    ps_r2_rain_drops_speed, 0.0f};
    string_unordered_map<const char*, Fvector4*> consts{
        {"rain_drops_params", &params}};
    RenderScreenQuad(Device.dwWidth, Device.dwHeight, dest_rt,
                     s_rain_drops->E[0], &consts);
...
void CRenderTarget::PhaseNightVision()
{
#if defined(USE_DX10) || defined(USE_DX11)
    ref_rt& dest_rt = RImplementation.o.dx10_msaa ? rt_Generic : rt_Color;
#else
    ...
    Fvector4 color{ps_pnv_color.x, ps_pnv_color.y, ps_pnv_color.z, ps_pnv_mode};
    Fvector4 params{ps_pnv_noise, ps_pnv_scanlines, 0.f, 0.f};
    string_unordered_map<const char*, Fvector4*> consts{
        {"pnv_color", &color}, {"pnv_params", &params}};

    RenderScreenQuad(Device.dwWidth, Device.dwHeight, dest_rt,
                     s_night_vision->E[0], &consts);
